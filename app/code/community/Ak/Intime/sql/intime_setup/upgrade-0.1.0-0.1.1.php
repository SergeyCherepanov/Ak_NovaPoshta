...
$installer->run("
CREATE TABLE {$this->getTable('intime_city')} (
...
CREATE TABLE {$this->getTable('intime_warehouse')} (
  `id` int(10) unsigned NOT NULL,
  ...
  `updated_at` timestamp NOT NULL default CURRENT_TIMESTAMP on update CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  CONSTRAINT FOREIGN KEY (`city_id`) REFERENCES `{$this->getTable('intime_city')}` (`id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
");
